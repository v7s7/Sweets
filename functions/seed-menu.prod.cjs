// functions/seed-menu.prod.cjs
// Seeds PRODUCTION Firestore (NOT the emulator).
delete process.env.FIRESTORE_EMULATOR_HOST;

const admin = require('firebase-admin');
// Path is from the functions/ dir → up into scripts/
const svc = require('../scripts/serviceAccount.json');
...
(async () => {
  ...
  for (const it of items) {
    batch.set(col.doc(it.id), { name: it.name, price: it.price });
  }
  await batch.commit();
